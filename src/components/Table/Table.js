...
export default function Table({languages, TestimonialsState}) {
    
    const dispatch  = useDispatch();
    
    const [ExerciseValue, setExerciseValue] = useState('');
    const [order, setOrder] = useState(TestimonialsState.order);

    const filterbyexercise = (value) => {
        dispatch(FilterTestimonialsByExercise(value));
    ...
    return (
        <div className=" mx-8 mb-10  ">
            <table className="table mt-5 border-1 border-gray-600 rounded-xl px-4 py-2 shadow-2xl">
                    ...
                    <tr>
                        <th className='lg:w-40 xl:w-40 py-0 font-normal'>
                            <div className="dropdown dropdown-top flex items-center ">
                                ...
                                <div>
                                    ...
                                    <ul tabIndex={0} className="dropdown-content z-[1]  h-60 overflow-y-scroll overflow-x-hidden menu p-2 w-auto shadow bg-base-100 rounded-box ">
                        ...
                        <th>
                            <select className="select w-full bg-gray-200 max-w-xs font-normal" onChange={(e) => handleChangeOrder(e.target.value)}>
                                ...
                                <option value="newest_ﬁrst" selected>newest ﬁrst</option>
                ...
                <tbody >
                {!(TestimonialsState.testimonials.length === 0) && TestimonialsState.testimonials.map((testimonial, key) => 
                    <tr >
